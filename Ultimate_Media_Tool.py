...
====
  ...
  - LIBRARIES / TOOLS only in:  C:\AKT Media Tools
      (SITE_PACKAGES, Tools, GoogleLibs)
  - MEDIA download + scraper in: folder of this .py file
  - Soft UAC only if library folder is not writable
  - URL sources order:
      1) Google Drive Google Docs (folder scan)
      2) Ultimate Media Tool URL Picker.txt
      3) Manual URL prompt (8s)
      4) Local media in script folder
  - Platform detect: Instagram / X / YouTube / other
  - Video downloading is cookie-independent (yt-dlp ladder; ignore browser cookies)
  - Original downloaded media → OCR/Whisper (NOT screenshots)
  - Instagram/X screenshots: S24 Ultra Playwright; timestamp Post_Saved_On injected+auto-fit
  - Green-screen overlay ONLY when visible video exists
      X: _Video2GreenScreen.png | IG: _GREEN_VIDEO.png / slide naming
  - Instagram carousels slide-by-slide; green only on video slides; rename slide1
  - YouTube: video-only (skip social screenshot system)
  - Other HTTP(S): best-effort screenshot + universal video download
  - Google Doc output: {DocTitle}/ with Image Scraper + Video Scraper + Screenshots/
  - Remove URL from Google Doc only after successful processing; archive locally
  - Groq multi-key: rotate at RPM 29/30; key-switch delay 600s
  - Models: vision qwen/qwen3.6-27b | translate llama-3.3-70b-versatile | whisper whisper-large-v3
====
"""
...
# INSTRUCTIONS
# ====
# 1. Put Groq API keys in USER_CONFIG (placeholders below — replace with your keys).
# 2. Place credentials.json (Google OAuth desktop) next to this script for Drive access.
# 3. Put social URLs in "Ultimate Media Tool URL Picker.txt" OR paste at 8s prompt.
# 4. SECURITY: never commit real API keys.
# ====
...
# ============================================================================
# ULTIMATE MEDIA TOOL FINAL — PATHS / GROQ / DRIVE
# ============================================================================
VERSION = "FINAL"
GOOGLE_LIBS = os.path.join(LIB_ROOT, "GoogleLibs")
JOB_LOG_FILE = Path(SCRIPT_DIR) / "Ultimate Media Tool Job Log.txt"
COMBINED_URL_ROOT = Path(SCRIPT_DIR) / "Combined Ultimate Media Tool"
COMBINED_IMAGE_DIR = Path(SCRIPT_DIR) / "Combined Image Scraper by AKT"
COMBINED_VIDEO_DIR = Path(SCRIPT_DIR) / "Combined Video Scraper by AKT"
COMBINED_AUDIO_DIR = Path(SCRIPT_DIR) / "Combined Audio Scraper by AKT"
...
GROQ_KEY_SWITCH_DELAY_SECONDS = KEY_SWITCH_DELAY_SECONDS
...
GROQ_MODELS = {
    ...
    "translate": list(TRANSLATE_MODELS),
...
MEDIA_EXTS = IMAGE_EXTS | VIDEO_EXTS_SCRAPER | AUDIO_EXTS
SIDECAR_EXTS = {
...
# ============================================================================
URL_PICKER_FILE = Path(SCRIPT_DIR) / "Ultimate Media Tool URL Picker.txt"
DONE_URL_LOG_FILE = Path(SCRIPT_DIR) / "Completed Ultimate Media Tool URL.txt"
ERROR_LOG_FILE = Path(SCRIPT_DIR) / "Ultimate Media Tool URL Picker Failed.txt"

# Aliases so FINAL helpers and snapshotter picker share the same paths
PICKER_FILE = URL_PICKER_FILE
PICKER_DONE_FILE = DONE_URL_LOG_FILE
PICKER_FAILED_FILE = ERROR_LOG_FILE
...
def is_placeholder_key(api_key: str) -> bool:
    return (not api_key) or ("YOUR_GROQ_API_KEY" in api_key) or api_key.endswith("_PLACEHOLDER")


def extract_api_keys() -> List[Dict[str, str]]:
    """Parse USER_CONFIG numbered Groq slots; skip placeholders."""
    ...
    for match in pattern.finditer(USER_CONFIG):
        ...
        if is_placeholder_key(api_key):
            continue
        accounts.append({"user_id": user_id, "api_key": api_key, "key": api_key})
    return accounts
...
def estimate_tokens_from_response(data: Dict[str, Any]) -> int:
    try:
...
class ApiKeyRotator:
    """Groq multi-key rotator: RPM rotate at 29, 600s key-switch delay."""

    KEY_SWITCH_DELAY_SECONDS = KEY_SWITCH_DELAY_SECONDS
    RPM_LIMIT = GROQ_RPM_LIMIT
    ROTATE_AT_RPM = GROQ_ROTATE_AT_RPM
    ...
    def __init__(self, entries: Sequence[Dict[str, str]]):
        ...
        for account in entries:
            key = account.get("api_key") or account.get("key") or ""
    ...
    def _refresh_window(self, account: Dict[str, Any]) -> None:
        now = time.time()
        if now - float(account["rpm_window"]) >= 60:
            account["rpm_window"] = now
            account["rpm_count"] = 0
        window = account["tpm_window"]
        while window and now - window[0][0] >= 60:
            window.popleft()

    def _tpm_used(self, account: Dict[str, Any]) -> int:
        self._refresh_window(account)
        return sum(t for _, t in account["tpm_window"])

    def _available(self, account: Dict[str, Any]) -> bool:
        self._refresh_window(account)
        if time.time() < float(account["disabled_until"]):
            return False
        if int(account["rpm_count"]) >= self.ROTATE_AT_RPM:
            return False
        if self._tpm_used(account) >= self.TPM_LIMIT:
            return False
        return True
    ...
    def current_account(self) -> Optional[Dict[str, Any]]:
        ...
        for offset in range(len(self.entries)):
            idx = (self.index + offset) % len(self.entries)
            account = self.entries[idx]
            if self._available(account):
                if idx != self.index:
                    self._switch_to(idx)
                return account
        wait_until = min(float(a["disabled_until"]) for a in self.entries)
    ...
    def _switch_to(self, idx: int) -> None:
        now = time.time()
        if self.last_switch_at and now - self.last_switch_at < self.KEY_SWITCH_DELAY_SECONDS:
            delay = self.KEY_SWITCH_DELAY_SECONDS - (now - self.last_switch_at)
            print_step("~", f"Groq key switch delay: sleeping {delay:.0f}s", C.DIM)
            time.sleep(delay)
        self.index = idx
        self.last_switch_at = time.time()
        account = self.entries[self.index]
        print_step("~", f"Switched Groq key to {mask_user_id(str(account['user_id']))}", C.CYAN)

    def mark_request(self, account: Dict[str, Any], tokens: int = 0) -> None:
        self._refresh_window(account)
        account["rpm_count"] = int(account["rpm_count"]) + 1
        account["rpd_count"] = int(account["rpd_count"]) + 1
        if tokens:
            account["tpm_window"].append((time.time(), int(tokens)))
        if int(account["rpm_count"]) >= self.ROTATE_AT_RPM:
            account["disabled_until"] = max(float(account["disabled_until"]), time.time() + 60)
            next_idx = (self.entries.index(account) + 1) % len(self.entries)
            if next_idx != self.index and len(self.entries) > 1:
                self._switch_to(next_idx)

    def mark_error(self, account: Dict[str, Any], status: Optional[int] = None) -> None:
    ...
    def balance_report(self) -> str:
        ...
        rows = []
        for i, account in enumerate(self.entries, 1):
            self._refresh_window(account)
            rows.append(
                f"{i}. {mask_user_id(str(account['user_id']))} "
                f"RPM {account['rpm_count']}/{self.RPM_LIMIT} "
                f"(rotate at {self.ROTATE_AT_RPM}) RPD {account['rpd_count']}"
            )
        return "\n".join(rows)


GROQ_KEY_MANAGER: Optional[ApiKeyRotator] = None
...
def call_with_key_rotation(make_request, *, label: str = "groq"):
    """Invoke make_request(api_key) with automatic key rotation on RPM/errors."""
    global GROQ_KEY_MANAGER
    rotator = GROQ_KEY_MANAGER
    if rotator is None or not rotator.has_keys():
        raise RuntimeError("No usable Groq API keys in USER_CONFIG")
    last_err = None
    for _ in range(max(3, len(rotator.entries) * 2)):
        account = rotator.current_account()
        if not account:
            raise RuntimeError("No Groq API key available")
        try:
            result = make_request(str(account["api_key"]))
            tokens = 0
            if isinstance(result, dict):
                tokens = estimate_tokens_from_response(result)
            rotator.mark_request(account, tokens)
            return result
            ...
            rotator.mark_error(account, status)
            print_step("!", f"{label} error on {mask_user_id(str(account['user_id']))}: {exc}", C.YELLOW)
            continue
    raise RuntimeError(f"{label} failed after key rotation: {last_err}")


def call_with_model_fallback(models: Sequence[str], make_request, *, label: str = "model"):
    """Try each model id until one succeeds (skip model-not-found)."""
    last_err = None
    for model in models:
        try:
            ...
            if "model" in low and ("not found" in low or "does not exist" in low or "404" in low):
                print_step("!", f"{label}: model {model} unavailable — trying next", C.YELLOW)
                continue
            raise
    raise RuntimeError(f"{label} failed all models: {last_err}")


def groq_json_request(method: str, endpoint: str, payload: Optional[Dict[str, object]] = None) -> Dict[str, object]:
    def _do(api_key: str):
        data = None
        headers = {
        ...
        request = urllib.request.Request(
            GROQ_API_BASE + endpoint, data=data, headers=headers, method=method
        )
        try:
    ...
    return call_with_key_rotation(_do, label=f"groq {method} {endpoint}")


def groq_multipart_request(endpoint: str, fields: Dict[str, str], file_field: str, file_path: Path) -> Dict[str, object]:
    def _do(api_key: str):
        ...
        request = urllib.request.Request(
            GROQ_API_BASE + endpoint, data=bytes(body), headers=headers, method="POST"
        )
        try:
            with urllib.request.urlopen(request, timeout=GROQ_REQUEST_TIMEOUT) as response:
    ...
    return call_with_key_rotation(_do, label=f"groq multipart {endpoint}")


def refresh_live_models_from_groq() -> Dict[str, str]:
    global LIVE_MODELS
    chosen = {
        "vision": VISION_MODELS[0],
        "translate": TRANSLATE_MODELS[0],
    ...
    if GROQ_KEY_MANAGER is None or not GROQ_KEY_MANAGER.has_keys():
        LIVE_MODELS = chosen
        return chosen
    try:
        data = groq_json_request("GET", "/models")
...
def groq_vision_ocr(model: str, path: Path) -> str:
    prompt = (
    ...
    data = groq_json_request("POST", "/chat/completions", payload)
...
def groq_translate_cleanup(model: str, text: str) -> str:
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "Clean OCR/transcript noise. Translate non-English to clear English. "
                    "Keep meaning. Return plain text only."
                ),
            },
...
def groq_whisper(model: str, path: Path) -> str:
...
def which_tool(name: str) -> Optional[str]:
    ...
    if path:
        return path
    if os.name == "nt":
        for candidate in (Path(TOOLS_DIR) / f"{name}.exe", Path(LIB_ROOT) / "bin" / f"{name}.exe"):
            if candidate.exists():
                return str(candidate)
    return get_ffmpeg_executable() if name == "ffmpeg" else None
...
def extract_audio_for_whisper(video_path: Path) -> Optional[Path]:
    ...
    if not ffmpeg:
        return None
    out = Path(tempfile.gettempdir()) / f"akt_umt_whisper_{os.getpid()}_{file_stamp()}.wav"
    cmd = [
        ffmpeg, "-y", "-i", str(video_path),
        "-vn", "-ac", "1", "-ar", "16000", "-f", "wav", str(out),
    ]
    try:
        subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=600, check=True)
        if out.exists() and out.stat().st_size > 44:
            return out
    except Exception as exc:
        print_step("!", f"ffmpeg extract failed: {exc}", C.YELLOW)
    return None
...
def unique_path(path: Path) -> Path:
    ...
    for i in range(2, 10000):
        ...
        if not candidate.exists():
            return candidate
    return path.with_name(f"{stem}_{file_stamp()}{suffix}")
...
def parse_datetime_text(value: object) -> Optional[datetime]:
    ...
    if not text:
        return None
    cleaned = text.replace("+00:00", "").rstrip("Z")
    for fmt in (
        "%Y:%m:%d %H:%M:%S",
        "%Y-%m-%d %H:%M:%S",
        "%Y-%m-%dT%H:%M:%S",
        ...
        try:
            return datetime.strptime(cleaned[:19] if "T" in cleaned or " " in cleaned else cleaned[:10], fmt)
        except Exception:
            continue
    try:
...
def media_sort_key(path: Path):
    return (media_datetime(path), str(path).lower())



# ============================================================================
# SCRAPER (OCR / Whisper) — screenshots NEVER enter here
# ============================================================================

def scrape_media_file(path: Path, models: Dict[str, str]) -> str:
    kind = classify_media(path)
    if GROQ_KEY_MANAGER is None or not GROQ_KEY_MANAGER.has_keys():
        return "Scraper skipped: USER_CONFIG contains placeholders only."
    try:
        if kind == "image":
            text = call_with_model_fallback(
                GROQ_MODELS["vision"],
                lambda m: groq_vision_ocr(m, path),
                label="vision",
            )
            return call_with_model_fallback(
                GROQ_MODELS["translate"],
                lambda m: groq_translate_cleanup(m, text),
                label="translate",
            )
        if kind in ("audio", "video"):
            audio_path = path if kind == "audio" else extract_audio_for_whisper(path)
            if not audio_path:
                return "Whisper skipped: ffmpeg not found or audio extraction failed."
            text = call_with_model_fallback(
                GROQ_MODELS["whisper"],
                lambda m: groq_whisper(m, audio_path),
                label="whisper",
            )
            if kind == "video" and audio_path != path:
                try:
...
def _scraper_targets(
    output_prefix: Optional[str],
    drive_doc_mode: bool,
    drive_doc_title: str = "",
) -> Dict[str, Tuple[Path, Path, str]]:
    """
    Always define folders/logs even when output_prefix is None
    (default Combined Image/Video/Audio Scraper by AKT).
    In drive_doc_mode: {DocTitle}/ with Image Scraper + Video Scraper
    ...
    if drive_doc_mode:
        ...
        video_log = doc_root / f"{title} Video Scraper.txt"
        return {
    ...
    if prefix:
        root = ensure_dir(COMBINED_URL_ROOT / safe_filename(prefix, 100))
        return {
            "image": (ensure_dir(root / "Combined Image Scraper by AKT"), root / "Combine Image Scraper.txt", "Image"),
            "video": (ensure_dir(root / "Combined Video Scraper by AKT"), root / "Combine Video Scraper.txt", "Video"),
            "audio": (ensure_dir(root / "Combined Audio Scraper by AKT"), root / "Combine Audio Scraper.txt", "Audio"),
        }

    return {
        "image": (ensure_dir(COMBINED_IMAGE_DIR), COMBINE_IMAGE_LOG, "Image"),
        "video": (ensure_dir(COMBINED_VIDEO_DIR), COMBINE_VIDEO_LOG, "Video"),
        "audio": (ensure_dir(COMBINED_AUDIO_DIR), COMBINE_AUDIO_LOG, "Audio"),
    }
...
def run_scraper(
    ...
    *,
    ...
    drive_doc_mode: bool = False,
    drive_doc_title: str = "",
    job_name: str = "",
) -> Tuple[int, int]:
    """
    Image→Groq Vision OCR; Video/Audio→ffmpeg extract + Whisper; translate;
    write Combine logs; naming Image N / Video N / Audio N.
    Sort by Date created earliest first.
    Screenshots must NOT be passed here — caller responsibility.
    """
    targets = _scraper_targets(output_prefix, drive_doc_mode, drive_doc_title)
    files: List[Path] = []
    if only_files is not None:
        files = [Path(p) for p in only_files if Path(p).exists() and is_media_file(Path(p))]
    else:
        for folder in (COMBINED_IMAGE_DIR, COMBINED_VIDEO_DIR, COMBINED_AUDIO_DIR, Path(SCRIPT_DIR)):
            if not folder.exists():
                continue
            for p in folder.iterdir():
                if is_media_file(p) and not sidecar_or_temp(p):
                    files.append(p)

    # Exclude anything under Screenshots folders
    files = [
        p for p in files
        if "screenshot" not in str(p).lower()
        and "_green_video" not in p.name.lower()
        and "_video2greenscreen" not in p.name.lower()
    ]
    files = sorted(files, key=media_sort_key)
    ...
    for source in files:
        kind = classify_media(source)
        if kind not in targets:
            continue
        folder, log_path, label = targets[kind]
        # In drive_doc_mode audio shares Video folder but keeps Audio naming
        counters[kind] = counters.get(kind, 0) + 1
        idx = counters[kind]
        ensure_dir(folder)
        dt = media_datetime(source)
        date_prefix = dt.strftime("%Y-%m-%d_%H-%M-%S")
        base = safe_filename(source.stem, 80)
        dest = unique_path(folder / f"{label} {idx} {date_prefix} {base}{source.suffix.lower()}")
        try:
            shutil.copy2(str(source), str(dest))
        except Exception as exc:
            print_step("!", f"Copy failed for {source.name}: {exc}", C.YELLOW)
            errors += 1
            continue
        print_step("+", f"{label} {idx}: {dest.name}", C.GREEN)
        scraped = scrape_media_file(dest, models)
        log_entry = [
            "=" * 72,
            f"{label} {idx}",
            f"Job: {job_name or output_prefix or 'local'}",
            f"Time: {now_stamp()}",
...
# ============================================================================
# VIDEO DOWNLOAD WRAPPER (cookie-independent) + SCREENSHOT STAGE
# ============================================================================

def download_media_from_url(url: str, dest_folder: Path) -> Tuple[List[Path], str, bool]:
    """
    Cookie-independent universal download via run_yt_dlp_download + recovery passes.
    Returns (paths, log, permanent_failure).
    """
    ...
    for attempt in range(1, 4):
        print_step("~", f"Download attempt {attempt}/3 (cookie-independent) for {platform}", C.CYAN)
        try:
            results = run_yt_dlp_download(
                url,
                str(dest_folder),
            ...
            for r in results or []:
                ...
                if p.exists() and p not in all_paths:
                    all_paths.append(p)
            logs.append(f"attempt {attempt}: {len(results or [])} file(s)")
            if all_paths:
                return all_paths, "\n".join(logs), False
        except Exception as exc:
            logs.append(f"attempt {attempt} error: {exc}")
            print_step("!", f"Download attempt {attempt} failed: {exc}", C.YELLOW)
...
def _looks_permanent_download_error(log: str) -> bool:
    low = (log or "").lower()
    hints = (
        "404", "not found", "private video", "account has been",
        "unsupported url", "no video formats", "http error 410",
        "has been removed", "copyright",
    )
    return any(h in low for h in hints)


async def capture_screenshots_only(url: str, output_dir: str) -> List[str]:
    """
    Instagram/X S24 Ultra screenshots (timestamp + green-screen rules).
    YouTube → [] (skip). Other HTTP(S) → best-effort page screenshot.
    Returns saved screenshot paths for logging only.
    """
    from playwright.async_api import async_playwright
    ...
    platform = detect_platform(url)
    if platform == "youtube":
        print_step("~", "YouTube: skipping social screenshot system", C.DIM)
        return []
    ...
    username = platform_username_from_url(url, platform)
    domain = (urlparse(url).hostname or "").lower()
    # Cookies ONLY for Playwright login walls — never for video download path
    cookies = get_browser_cookies_for_domain(domain) if domain else []
    ig_cookies = get_instagram_cookies() if is_instagram else cookies
    saved: List[str] = []
    ...
    async with async_playwright() as p:
        ...
        try:
            if is_twitter:
                match = re.search(r"(?:twitter\.com|x\.com)/([^/]+)/status/(\d+)", url)
                tw_username = match.group(1) if match else (username or "unknown")
                tweet_id = match.group(2) if match else "0"
                filename_base = f"x.com_@{tw_username}_{timestamp_str}"
                target_url = f"https://platform.twitter.com/embed/Tweet.html?id={tweet_id}&theme=light"
                context = await new_mobile_context(browser, S24_UA, None)
                page = await context.new_page()
                await page.goto(target_url, wait_until="domcontentloaded", timeout=60000)
                await page.wait_for_selector("article", state="visible", timeout=15000)
                await page.wait_for_timeout(3000)
                ...
                try:
                    if await screenshot_with_video_green(
                        ...
                        saved.append(green_output_path)
                        print(f"    [+] Green-screen video screenshot saved: {os.path.basename(green_output_path)}")
                    elif os.path.exists(green_output_path):
                        os.remove(green_output_path)
                except Exception as green_err:
            ...
            if is_instagram:
                context = await new_mobile_context(browser, S24_FIREFOX_UA, ig_cookies)
                page = await context.new_page()
                paths = await capture_instagram(page, url, output_dir, timestamp_str, timestamp_display)
                still_blank = paths and all(screenshot_looks_blank(p) for p in paths)
                if still_blank:
                    await browser.close()
                    browser = await launch_installed_chrome_family(p)
                    context = await new_mobile_context(browser, S24_CHROME_UA, ig_cookies)
            ...
            # Other HTTP(S): best-effort screenshot
            context = await new_mobile_context(browser, S24_FIREFOX_UA, cookies)
            page = await context.new_page()
            await page.goto(url, wait_until="domcontentloaded", timeout=60000)
            await page.wait_for_timeout(2500)
            ...
            filename_base = safe_filename(
                f"{PLATFORM_DOMAIN_MAP.get(platform, platform)}_{username}_{timestamp_str}"
            )
            output_path = os.path.join(output_dir, f"{filename_base}.png")
...
def run_screenshot_stage(url: str, screenshots_dir: Path) -> List[Path]:
    """Sync wrapper — returns screenshot paths for logging only (never pass to scraper)."""
    ensure_dir(screenshots_dir)
    try:
        paths = asyncio.run(capture_screenshots_only(url, str(screenshots_dir)))
        return [Path(p) for p in (paths or []) if p and os.path.exists(p)]
    except Exception as exc:
        print_step("!", f"Screenshot stage failed: {exc}", C.YELLOW)
        append_text(PICKER_FAILED_FILE, f"{now_stamp()} SCREENSHOT_FAIL {url} :: {exc}")
        return []


def ensure_screenshot_environment() -> None:
    """Prepare Playwright / font / tools for screenshot stage."""
    try:
        prepare_environment()
    except Exception as exc:
        print_step("!", f"Screenshot environment warning: {exc}", C.YELLOW)


def setup_tools() -> None:
    """Ensure site-packages path, video deps, picker files."""
    if SITE_PACKAGES not in sys.path:
        sys.path.insert(0, SITE_PACKAGES)
    if GOOGLE_LIBS not in sys.path:
...
def _ensure_lib_folder() -> None:
    """Create AKT lib folders; soft UAC on Windows if not writable."""
    if os.name == "nt":
        def can_write(folder: str) -> bool:
            try:
                os.makedirs(folder, exist_ok=True)
                probe = os.path.join(folder, f".write_test_{os.getpid()}.tmp")
        ...
        if not can_write(LIB_ROOT) and os.environ.get("AKT_UMT_ELEVATED") != "1":
            print_step("!", f"{LIB_ROOT} not writable — requesting soft UAC...", C.YELLOW)
            try:
                ...
                print_step("!", f"UAC skipped: {exc}", C.YELLOW)
    ensure_dir(LIB_ROOT)
    ensure_dir(SITE_PACKAGES)
    ensure_dir(TOOLS_DIR)
    ensure_dir(GOOGLE_LIBS)
...
def install_packages_soft() -> None:
    """Install REQUIRED + GOOGLE packages only if import fails."""
    _ensure_lib_folder()
    if SITE_PACKAGES not in sys.path:
        sys.path.insert(0, SITE_PACKAGES)
    if GOOGLE_LIBS not in sys.path:
    ...
    def _need(module: str) -> bool:
        try:
            __import__(module.split(".")[0] if module.startswith("google.") else module)
            return False
        except Exception:
            # google.auth special
            try:
    ...
    for module, pip_name in REQUIRED_PACKAGES_UMT:
        if _need(module):
            print_step("~", f"Installing {pip_name} → {SITE_PACKAGES}", C.DIM)
            try:
                subprocess.check_call(
                    [
                        sys.executable, "-m", "pip", "install", "--upgrade",
                        "--target", SITE_PACKAGES, "--no-warn-script-location", pip_name,
                    ]
                )
            except Exception as exc:
                print_step("!", f"Install failed {pip_name}: {exc}", C.YELLOW)

    for module, pip_name in GOOGLE_PACKAGES:
        if _need(module):
            print_step("~", f"Installing {pip_name} → {GOOGLE_LIBS}", C.DIM)
            try:
                subprocess.check_call(
                    [
                        sys.executable, "-m", "pip", "install", "--upgrade",
                        "--target", GOOGLE_LIBS, "--no-warn-script-location", pip_name,
                    ]
                )
            except Exception as exc:
                print_step("!", f"Google lib install failed {pip_name}: {exc}", C.YELLOW)
...
def _get_google_credentials():
    if not _google_libs_available():
        raise RuntimeError("Google Drive libraries missing (install into GoogleLibs)")
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            if not GOOGLE_CREDENTIALS_FILE.exists():
                raise RuntimeError(
                    f"Missing {GOOGLE_CREDENTIALS_FILE.name} for Google OAuth "
                    "(Drive readonly + Documents scopes)"
                )
...
def _list_google_docs_in_folder(folder_id: str) -> List[Dict[str, str]]:
    ...
    while True:
        ...
        for f in resp.get("files", []):
            docs.append({"id": f["id"], "name": f.get("name") or "Untitled"})
...
def _extract_urls_from_google_doc(doc_id: str) -> List[str]:
    docs = _docs_service()
                ...
                for pe in el["paragraph"].get("elements", []):
                    ...
                    for m in re.finditer(r"https?://[^\s<>\"'\])\}]+", text, re.I):
                        ...
                        if u.lower() not in seen:
                            seen.add(u.lower())
                            urls.append(u)
                    style = (tr.get("textStyle") or {}).get("link") or {}
                    link = style.get("url")
                    if link and link.lower() not in seen and link.startswith("http"):
                        seen.add(link.lower())
                        urls.append(link)
            if "table" in el:
...
def _remove_url_from_google_doc(doc_id: str, url: str) -> bool:
    """Remove URL text occurrences from the Google Doc after successful processing."""
    try:
        docs = _docs_service()
        document = docs.documents().get(documentId=doc_id).execute()
        ...
        def find_ranges(elements, url_text: str):
            found = []
            for el in elements or []:
                if "paragraph" in el:
                    for pe in el["paragraph"].get("elements", []):
                        ...
                        if url_text in text:
                            ...
                            # replace just the URL substring if possible
                            if start is not None:
                                idx = text.find(url_text)
                                if idx >= 0:
                                    found.append((start + idx, start + idx + len(url_text)))
                if "table" in el:
                    for row in el["table"].get("tableRows", []):
                        for cell in row.get("tableCells", []):
                            found.extend(find_ranges(cell.get("content", []), url_text))
            return found

        ranges = find_ranges(content, url)
        ...
        if not requests:
            # Fallback: replaceAllText
            requests = [{
                "replaceAllText": {
                    "containsText": {"text": url, "matchCase": True},
                    "replaceText": "",
                }
            }]
        docs.documents().batchUpdate(documentId=doc_id, body={"requests": requests}).execute()
        print_step("+", f"Removed URL from Google Doc after success", C.GREEN)
        return True
    except Exception as exc:
        print_step("!", f"Could not remove URL from Google Doc: {exc}", C.YELLOW)
        return False
...
def _process_one_drive_url(url: str, doc_id: str, doc_title: str) -> bool:
    ...
    processed, errors = run_scraper(
        only_files=media,
        drive_doc_mode=True,
        drive_doc_title=doc_title,
        job_name=f"Drive:{doc_title}",
    )
    append_text(
        JOB_LOG_FILE,
...
def _process_google_drive_docs() -> bool:
    """
    HARD GATE scan: returns True if Drive libs OK and folder scan succeeded
    (even when zero docs / zero work). Returns False if libs missing or scan fails
    — caller must STOP and not touch picker.
    """
    print_step("~", "GOOGLE DRIVE FIRST — scanning docs...", C.CYAN)
    if not _google_libs_available():
        print_step("!", "Google Drive libraries missing — stopping (picker not touched).", C.RED)
        return False
    try:
        docs = _list_google_docs_in_folder(GOOGLE_DRIVE_FOLDER_ID)
    except Exception as exc:
        print_step("!", f"Google Drive scan failed: {exc} — stopping (picker not touched).", C.RED)
        append_text(JOB_LOG_FILE, f"{now_stamp()} DRIVE_SCAN_FAIL {exc}\n{traceback.format_exc()}\n")
        return False

    print_step("+", f"Drive scan OK — {len(docs)} Google Doc(s) in folder", C.GREEN)
    for doc in docs:
        ...
        for url in pending:
            try:
                _process_one_drive_url(url, doc_id, title)
            except Exception as exc:
                print_step("!", f"Drive URL job failed: {exc}", C.YELLOW)
                append_text(JOB_LOG_FILE, f"{now_stamp()} DRIVE_URL_FAIL {title} {url} :: {exc}\n")
    # Scan succeeded — CONTINUE to picker even if Drive had work
    return True


# ============================================================================
# URL PICKER / MANUAL / LOCAL
# ============================================================================

def archive_completed_url(url: str) -> None:
    record_completed_url(url)  # uses PICKER_DONE_FILE alias via DONE_URL_LOG_FILE


def quarantine_url_as_failed(url: str, reason: str = "") -> None:
    append_text(PICKER_FAILED_FILE, f"{now_stamp()} FAIL {url} :: {reason}")
    print_step("!", f"Quarantined permanent failure → {PICKER_FAILED_FILE.name}", C.YELLOW)


def write_job_log(message: str) -> None:
    append_text(JOB_LOG_FILE, f"{now_stamp()} {message}")


def get_url_with_timeout_umt(seconds: int = 8) -> str:
    """8s manual URL prompt — msvcrt on Windows, input fallback elsewhere."""
    print_step("~", f"Enter URL ({seconds}s auto-skip if empty):", C.CYAN)
    if HAS_MSVCRT and sys.stdin.isatty():
        ...
        try:
            import select
            print(f"(waiting up to {seconds}s) ", end="", flush=True)
            ready, _, _ = select.select([sys.stdin], [], [], seconds)
            text = sys.stdin.readline().strip() if ready else ""
        except Exception:
            text = ""
...
def process_picker_url(url: str) -> bool:
    ...
    if success:
        remove_url_from_picker(url)
        archive_completed_url(url)
        write_job_log(f"PICKER_OK {url} media={len(media)} shots={len(shots)}")
        if GROQ_KEY_MANAGER:
...
def local_media_files() -> List[Path]:
    found = []
    for p in Path(SCRIPT_DIR).iterdir():
        if is_media_file(p) and not sidecar_or_temp(p):
            found.append(p)
...
def print_banner() -> None:
    ...
    print(color("ULTIMATE MEDIA TOOL FINAL - GOOGLE DRIVE + DOC CLEANUP", C.BOLD + C.CYAN))
    print(color("=" * 72, C.CYAN))
    print(f"Libraries/tools: {LIB_ROOT}")
    print(f"GoogleLibs: {GOOGLE_LIBS}")
    print(f"Media/script folder: {SCRIPT_DIR}")
    print(f"Combined root: {COMBINED_URL_ROOT}")
    print(f"URL picker: {PICKER_FILE.name}")
    print(f"Vision: {LIVE_MODELS.get('vision')}")
    print(f"Translate: {LIVE_MODELS.get('translate')}")
    print(f"Whisper: {LIVE_MODELS.get('whisper')}")
...
def main() -> None:
    ...
    if os.name == "nt":
        try:
            os.system("title ULTIMATE MEDIA TOOL FINAL")
        except Exception:
            pass
    ...
    try:
        ...
        GROQ_KEY_MANAGER = ApiKeyRotator(keys)
        ...
        if not GROQ_KEY_MANAGER.has_keys():
            print_step(
                "!",
                "USER_CONFIG has only placeholder Groq keys — downloads/screenshots work; OCR/Whisper skipped.",
                C.YELLOW,
            )

        # 1) HARD GATE: Google Drive first
        drive_ok = _process_google_drive_docs()
        if not drive_ok:
            print_step("!", "Drive hard-gate failed — exiting without touching URL picker.", C.RED)
            return
        ...
        if picker_urls:
            print_step("~", f"URL picker has {len(picker_urls)} URL(s) — processing", C.CYAN)
            for url in list(picker_urls):
                print_step("~", f"Picker: {url}", C.BLUE)
                try:
                    process_picker_url(url)
                except Exception as exc:
