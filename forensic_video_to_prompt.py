...
====
  ...
  - Download: direct video via urllib | Instagram/social via yt-dlp (+ gallery-dl fallback)
====
"""
...
{C.CYAN}{C.BOLD}
 ...
 |   Download: yt-dlp video + gallery-dl fallback                 |
 +----+
{C.RESET}""")
...
def download_video_from_url(url: str, dest_folder: Path) -> Tuple[List[Path], str, bool]:
    ...
    Smart download for videos:
      ...
      2. Social / Instagram reels & posts → yt-dlp first, then gallery-dl
    ...
    if _is_social_media_url(url):
        # Video tool: prefer yt-dlp (actual video), then gallery-dl
